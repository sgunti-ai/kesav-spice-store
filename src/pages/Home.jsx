  ...
  return (
    <main className="container">
      <section className="hero">
        ...
        <Link to="/products" className="btn-primary hero-cta">SHOP NOW</Link>
      </section>
      <section className="categories" aria-label="Catalog snippets">
        <div style={{ marginTop: 24 }}>
          <h3>Dry Fruits & Nuts</h3>
          <div className="grid-3">
            {CATALOG.dryFruits.slice(0,3).map(p => (
              <ProductCard key={p.id} product={p} price={initialPrices[p.priceKey]} onAdd={()=>{}} onView={handleView} />
            ))}
          </div>
        ...
        <div style={{ marginTop: 24 }}>
          <h3>Spices</h3>
          <div className="grid-3">
            {CATALOG.spices.slice(0,3).map(p => (
              <ProductCard key={p.id} product={p} price={initialPrices[p.priceKey]} onAdd={()=>{}} onView={handleView} />
            ))}
          </div>
